...
- **做了**：新增「目前候選股對照」表，將最新候選股身上的歷史訊號、平均報酬、
  勝率與樣本數直接顯示在網頁上
- **注意**：目前 `confidence=low`，樣本仍少；畫面已明確標示這是候選股事件紀錄回測，
  不是全市場完整回測，也不是穩定預測模型
- **已驗證**：`node --check frontend/app.js` 通過；`signal_backtest.json` 有 56 筆訊號摘要、
  40 筆候選股對照
- **建議下一步**：累積 30～60 個交易日後，再用這個頁面挑出可調高權重的訊號；
  若需要，可以把高勝率訊號反寫回左側/右側評分權重
...
- **做了**：回測 7/12~7/21 自選潛伏股 vs history，發現今日漲停的揚博/景碩/創意/凌巨
  **全部提前進過左側名單**，共同勝利訊號＝`short_covering`（借券回補）＋`margin_flush`
  （融資大減）100% 同時出現，但都卡在 70 門檻下（43~63 分）不顯眼
- **做了**：據此新增「🔥 深度洗盤」組合訊號——兩訊號同時且夠深（借券回補 ≥20%、
  融資 ≥12%，門檻高於各自單獨的 15/10）給 `washout_combo_bonus`（預設 10 分）＋
  獨立 reason `deep_washout`；前端名稱旁加 🔥 徽章（⚡ 訊號加速也一併加徽章）
- **驗證**：6 檔回測全部觸發，揚博 63.5→73.5 直接進正式候選；pytest 60 passed、node --check 通過
- **注意**：這是刻意的綜效加分（兩構面已各自計分，組合再加 10），高勝率但需觀察誤報率；
  門檻在 config 可調（washout_short_drop_pct／washout_margin_drop_pct／washout_combo_bonus）
- **建議下一步**：跑一輪看 🔥 命中檔數與隔日表現；若誤報多就把 bonus 降到 6~8 或拉高幅度門檻
